...
# to remove: giveaway_,millable, 20151002, parametric, customblock, BAT, propeller, lego-motor, lego_bracket
...
def main():
    ...
    for zip_path in zip_file_paths:
        ...
        print('\t'.join(fnames))
    print('Done Blacklisting. Files Blacklisted:')


def unzip_and_filter(path):
    with zipfile.ZipFile(path, 'r') as zip_ref:
        files = zip_ref.namelist()
        stl_files = [f for f in filter(lambda x: x.lower().endswith('.thing'), files)]
